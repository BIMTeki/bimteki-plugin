...
description: 在 BIMTeki 專案中生成「各層容積檢討」表格（每一地上樓層一張 TableTemplate 表格樣板，俗稱 XX層容積檢討／地上X層容積檢討）。當使用者想要「做各層容積檢討表 / 建立地上層容積檢討 / 產生每層容積檢討 / 幫這案做容積檢討表格 / per-floor volume review table / FAR review table per storey」時務必使用本 skill，即使沒有明講「skill」二字。表格為三欄式（區段標籤／項目名／值），由上而下分「當層樓地板面積(A)、免計容積(B)、回計容積(C)、(A)-(B)+(C)」四段；值欄全部用 storyArea 的「算式：」autotext 綁定、由 BIMTeki 即時計算，不寫死數字。本 skill 會先用 BIMTeki MCP 判案型（get_project_core_snapshot 的 building.case_type，含 is_row_house 連棟透天旗標）與樓層清單（get_project_stories 的 isBetweenFloor 夾層旗標），再依案型組表：非共用移除梯廳與 10%/15% 回計列；一般多棟每層用「各棟總計」樓層做單張表，並把室內面積、陽台面積、陽台＞2m 展開為各棟＋小計分列（同 autotext、樓層屬性各指到該棟該層）；連棟透天每層做一張多欄表（標題列後插入棟別列、每棟一個值欄，各欄綁該棟該層樓層屬性）；**任何案型只要該地上層存在配對的「…夾層」樓層，一律改用夾層(mezz)版面合併成一張表（單棟4欄 mezz_single、一般多棟5欄 mezz），不可因為是單棟就當成三個獨立樓層各自建表**；每層再用 evaluate_story_autotext_values 判陽台與各免計/回計項目是否為 0 決定出現與否（陽台面積為 0 時，陽台面積、陽台>2M、室內1/8或8m²、陽台1/8檢討、陽台最終面積 等陽台相關列整組不放；勾選「工廠類建築（陽台全額計入容積）」設定時，A 段只放 室內面積、陽台面積、當層樓地板面積 三列）。用 scripts/build_volume_table.py 產生 cells/merges，create_project_table_template 建表、modify_project_table_template 補合併與欄寬，最後讀回驗證。屋突層另有形式、不建（另有 bimteki-rooftop-area-review-table skill）；地下層若採地下層總量檢討則不建。本 skill 只負責「各層容積檢討表」；各層樓地板面積總表、基地概要、建照審查、土管、無障礙、容積區域繪製另有各自的 skill，不要用本 skill 處理。
---
