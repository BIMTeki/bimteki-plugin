...
3. **以上都沒有對應欄位的資料** → `set_project_custom_params` 寫成「使用者自訂義參數」。每筆的「項目名稱」全案唯一，且**自動變成 autotext 欄位鍵 `CustomParam.{name}`**，檢討表可以直接綁它。使用者點名的「地質敏感地區」「都市計畫發布日期與函文」多屬此類。
   - **綠化規定值也走這條，且名稱固定**：分類「綠化面積檢討」→「綠化比率」（`50%`）、「喬木檢討基準」（`64`）、「實設喬木數量」（`3`）、「檢討基數」（`實設空地`／`法定空地`）。綠化面積檢討表的公式（以 `custom-area-review` 建）綁這幾個名字，外掛的舊檔轉換也產生同名項目，不要自創近義名；細節見 `references/field-routing.md` C 節。
