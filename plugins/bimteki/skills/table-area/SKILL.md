...
前置檢查、唯讀工具、autotext 原則、儲存格格式、建表共通流程與寫入底線，統一收在 `references/spoke-conventions.md`（各 spoke 都指回那一份）。路由前最值得先提醒的幾點，都是本類表特有的坑：
