...
DEFAULT_TOKENS = {
    ...
    "算式：安全梯和管委會空間": "${TJL$F09DF9BDF9548}",
    "算式：機電設備空間和管道間": "${TJL$F29265BDA5983}",
...
ITEM_B = {
    "梯廳":      ("梯廳：",   ["算式：梯廳"]),
    "安全梯和管委會空間":   ("安全梯和管委會空間：",   ["算式：安全梯和管委會空間"]),
    "機電設備空間和管道間": ("機電設備空間和管道間：", ["算式：機電設備空間和管道間"]),
    "停車空間":  ("停車空間", ["算式：停車空間"]),
    "騎樓":      ("騎樓",    ["算式：騎樓"]),
    "車道":      ("車道",    ["算式：車道"]),
    "防空避難室": ("防空避難室", ["算式：防空避難室"]),
}
# 外掛 v0.0.21 前的舊項目名 → 新名（舊 config 的 section_b 仍可用）
ITEM_B_ALIASES = {"機電設備": "安全梯和管委會空間", "管道間": "機電設備空間和管道間"}
ITEM_C = {
...
def build(cfg):
    ...
    mode = cfg["mode"]
    sec_b = [ITEM_B_ALIASES.get(k, k) for k in cfg.get("section_b", [])]
