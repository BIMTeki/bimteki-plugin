...
description: 在 BIMTeki 專案中生成「基地概要」表格（TableTemplate 表格樣板，俗稱基地資訊表／建蔽容積規定值vs設計小表，常見於建照圖說 A0-01）。當使用者想要「做基地概要表 / 建立基地資訊表 / 產生基地概要 / 基地概要檢討 / 做建蔽容積規定值與設計值對照表 / site overview table / site summary table」時務必使用本 skill，即使沒有明講「skill」二字。表格為兩欄式：第一列為置中大標題「基地概要」，第二列起每列一項；左欄為項目標籤（基地地號、使用分區、起造人、層棟戶數、基地面積、**退縮／扣除列（保留地／道路退縮地／道路退縮地(計入法空)／鄰房侵占／騎樓／騎樓地，依本案面積>0 者動態插入，列數 0~6）**、使用面積、**使用面積(建蔽率用)（有扣除項時才出現，顯示如 93.00-1.38-14.34=77.28m²）**、建蔽率、容積率、允建建築面積、法定空地面積、設計建築面積、設計建蔽率、總樓地板面積、允建/設計容積樓地板面積、設計容積率、工程造價、雜項工程造價、總工程造價，以及停車空間、污水處理、無障礙、衛生設備、地質鑽探、防空避難、雨水滯洪等7項法規檢討），右欄為依本案填寫的內容。右欄組法優先序：**能對應到自動文字（含「算式：」「計算式：」等已含公式的複合 token）者優先綁 autotext，讓 BIMTeki 即時計算；找不到複合 token 時，用多個數值 autotext 搭配文字運算符號組成算式；工程造價單價依當年度地方主管機關公告、屬外部資訊，先問使用者並經同意後用 set_project_core_info(cost=...) 寫回專案再綁 autotext，不提供才留佔位符；法規檢討列依門檻值判斷後填標準樣板句**。再用 create_project_table_template 一步建表、modify_project_table_template 補樣板名與欄寬並讀回驗證。本 skill 只負責「基地概要表」；各層樓地板面積總表、建照審查表、土管檢討表、無障礙建築檢討表另有各自的 skill，不要用本 skill 處理。
---
